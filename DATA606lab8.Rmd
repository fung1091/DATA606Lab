...
This is an obervational study, the beauty rating is only an study by subjective observation without control and experimental process. It can not form the relationship between explanatory and response variables. Only it can have a correlation from low to high rating.

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?
    
    Yes, the distribution of score is skewed left, the students rate are tend to rate higer evaluations than lower evaluations. We may expect the normal distribution or less skewed because fewer professors will give the lowest or highest marks.
...
Using boxplots of visualization, it doesnot shown strong relationship between age and score, but it seem that the score tend to be lower after age of 59, but it is no difference for other age group.
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    it has shown the duplicate scores that were overlapping in the original plot.
```{r}
...
The average beauty rating is a a statistically significant predictor of evaluation score with p-value close of 0.
...
Linearity: It cannot be shown in linear relationship.
...
The residuals model is closely as normal values.

```{r}
plot(abs(m_bty_gen$residuals) ~ m_bty_gen$fitted.values)

```
The variability of the residuals against fitted values is nearly constant.


8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?

bty_avg is still a significant predicator of score for comparsion the distribution, and by using mulitline to separate the gender with 2 lines, the parmater will be changed between score and bty_avg.
By using the multil
...
Males tend to have a higher evaluation score than females who get the same rating.
...
I would expect the gender of professors to have the highest p-value in this model, 
...
I show that the cls_profssingle, the single course of professors, has the highest impact on score with a p-value of 0.77806

13. Interpret the coefficient associated with the ethnicity variable.

The p-value of "ethnicity not minority" is 0.11698 and parameter is 0.1234, it is positive slope and not a major affect the p-value.
...
```{r}
m_full_drop_profs <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_drop_profs)
```


This is slightly changed in coefficients and p-valued duing removing cls_profs. It is more significant to the level than before.
...
# remove rank, student, level and outfit
m_backSelect <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_credits + bty_avg 
             + pic_color, data = evals)
summary(m_backSelect)

coeffBackSelect <- coefficients(m_backSelect)
```

[ \begin{aligned} \widehat{score} = & r coeffBackSel[1] + r coeffBackSel[2] \text{ ethnicity}{not minority} + r coeffBackSel[3] \text{ gender}{male} + \ & r coeffBackSel[4] \text{ language}{non-english} + r coeffBackSel[5] \text{ age} + r coeffBackSel[6] \text{ cls_perc_eval } + \ & r coeffBackSel[7] \text{ cls_credits}{one credit} + r coeffBackSel[8] \text{ bty_avg} + r coeffBackSel[9] \text{ pic_color}_{color}\end{aligned} ]

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.

```{r}
qqnormSim(m_backSelect$residuals)
qqline(m_backSelect$residuals)
plot(abs(m_backSelect$residuals) ~ m_backSelect$fitted.values)
```
The residuals of the model is not normal distribution as residual values.

The variability of the residuals is nearly constant with fitted values.
...
This new information could not have an impact on any of conditions, because the class course are independent evaluated from each others.

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.

It would be high evaluation score when the professor is not minority, gender is male, received english education, teaching one credit course. And if professor get a high score from students with black and white class photo, relatively young and good percentage of completed evaluation.


19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?

The sample size of population are only six students, it would not be comfortable generalling the conclustion at any university, it should take more sample from other universities and add more factories.
